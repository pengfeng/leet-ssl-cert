...
class AliyunCLBDeployer(CertificateDeployer):
    ...
    def validate_credentials(self) -> None:
        ...
        try:
            self._client_or_raise().describe_server_certificates(
                request_cls(region_id=self._region_id())
            )
        except DeployError:
            raise
        except Exception as exc:
            raise DeployError(
                f"Alibaba Cloud CLB credential validation failed: {exc}"
            ) from exc
    ...
    def upload_certificate(self, name: str, cert_pem: str, key_pem: str) -> str:
        ...
        certificate_id = getattr(
            getattr(response, "body", response), "server_certificate_id", None
        )
        if not certificate_id:
            raise DeployError(
                "Alibaba Cloud CLB upload succeeded without returning server_certificate_id"
            )
    ...
    def bind_certificate(self, certificate_id: str) -> DeployResult:
        ...
        old_certificate_id = self._get_existing_certificate_id(
            load_balancer_id, listener_port
        )

        request_cls = self._import_request(
    ...
    def cleanup_old_certificates(self, name: str, keep: int = 1) -> list[str]:
        request_cls = self._import_request("DescribeServerCertificatesRequest")
        response = self._client_or_raise().describe_server_certificates(
            request_cls(region_id=self._region_id())
        )
        records = (
            getattr(
                getattr(response.body, "server_certificates", None),
                "server_certificate",
                [],
            )
            or []
        ...
        matching = [
            ...
            if str(getattr(record, "server_certificate_name", "")).startswith(prefix)
        ]
        matching.sort(
            key=lambda item: str(getattr(item, "server_certificate_name", "")),
            reverse=True,
        )
        ...
        for record in matching[keep:]:
            ...
            self._client_or_raise().delete_server_certificate(
                delete_request_cls(
                    region_id=self._region_id(), server_certificate_id=certificate_id
                )
            )
            deleted_ids.append(certificate_id)
        return deleted_ids

    def _get_existing_certificate_id(
        self, load_balancer_id: str, listener_port: int
    ) -> str | None:
        request_cls = self._import_request(
            "DescribeLoadBalancerHTTPSListenerAttributeRequest"
        )
        response = (
            self._client_or_raise().describe_load_balancer_httpslistener_attribute(
                request_cls(
                    region_id=self._region_id(),
                    load_balancer_id=load_balancer_id,
                    listener_port=listener_port,
    ...
    def _build_client(self) -> Any:
        ...
        if not access_key_id or not access_key_secret:
            raise DeployError(
                "aliyun_clb deployer requires access_key_id and access_key_secret"
            )
        try:
            ...
            raise DeployError(
                "Alibaba Cloud SLB SDK is not installed. Install leet-ssl-cert[aliyun]."
            ) from exc
    ...
    def _region_id(self) -> str:
        ...
        if not region_id:
            raise DeployError(
                "aliyun_clb deployer requires region or ALIBABA_CLOUD_REGION_ID"
            )
        return region_id
