...
class AcmeCertificateManager:
    ...
    def issue_certificate(
        self, certificate: CertificateConfig, dns_provider: DNSProvider
    ) -> IssuedCertificate:
        ...
        private_key_pem = self.storage.load_private_key_pem(
            certificate.name
        ) or self._generate_private_key_pem(self.acme.key_size)
        ...
        try:
            for authorization in order.authorizations:
                ...
                challenge_body = self._select_dns_challenge(
                    authorization.body.challenges
                )
                response, validation = challenge_body.chall.response_and_validation(
                    account_key
                )
                ...
                challenge_records.append(
                    ChallengeRecord(
                        zone=zone, record_name=record_name, value=validation
                    )
                )
            ...
            if not finalized.fullchain_pem:
                raise ACMEError(
                    f"ACME order for {certificate.name} did not return a certificate chain"
                )
            ...
            original_error = exc
            raise ACMEError(
                f"ACME issuance failed for {certificate.name}: {exc}"
            ) from exc
        except Exception as exc:
            original_error = exc
            raise ACMEError(
                f"Unexpected ACME failure for {certificate.name}: {exc}"
            ) from exc
        finally:
            cleanup_error = self._cleanup_challenge_records(
                dns_provider, challenge_records
            )
    ...
    def revoke_certificate(self, stored: StoredCertificate, reason: int = 0) -> None:
        ...
        try:
            ...
            raise ACMEError(
                f"Failed to revoke certificate {stored.name}: {exc}"
            ) from exc

    def _connect_client(self, account_key: jose.JWKRSA) -> acme_client.ClientV2:
        network = acme_client.ClientNetwork(
            key=account_key, user_agent="leet-ssl-cert/0.1.0"
        )
    ...
    def _cleanup_challenge_records(
        self, dns_provider: DNSProvider, records: list[ChallengeRecord]
    ) -> DNSError | None:
        errors: list[str] = []
        for record in reversed(records):
            try:
                dns_provider.delete_txt_record(
                    record.zone, record.record_name, record.value
                )
            except Exception as exc:  # pragma: no cover - cleanup best effort
                errors.append(f"{record.record_name}: {exc}")
        if errors:
            return DNSError(
                "Failed to clean up DNS challenge records: " + "; ".join(errors)
            )
        return None
